...
def seleccionar_lista(context,nombre_card):
    context.elements._implicity_wait(time=os.getenv("TIME_IMPLICITY"), element_by=PageModel.card_element).click()


@when(u'ingreso nombre de usuario "{nombre}"')
def seleccionar_nombre(context, nombre):
    context.elements._implicity_wait(time=os.getenv("TIME_IMPLICITY"), element_by=PageModel.txt_first_name).send_keys(nombre)


@when(u'ingreso correo "{correo}"')
def seleccionar_correo(context, correo):
    context.elements._implicity_wait(time=os.getenv("TIME_IMPLICITY"), element_by=PageModel.txt_email).send_keys(correo)


@when(u'ingreso direccion "{direccion}"')
def seleccionar_direccion(context,direccion):
    context.elements._implicity_wait(time=os.getenv("TIME_IMPLICITY"), element_by=PageModel.txt_direccion).send_keys(direccion)
...
def seleccionar_direccion_permanente(context,direccion_permanente):
    context.elements._implicity_wait(time=os.getenv("TIME_IMPLICITY"), element_by=PageModel.txt_direccion_permanente).send_keys(direccion_permanente)


@when(u'selecciono opcion enviar')
def seleccionar_enviar(context):
    context.elements._implicity_wait(time=os.getenv("TIME_IMPLICITY"), element_by=PageModel.btn_enviar).click()
...
def validar_formulario(context,nombre,correo,direccion,direccion_permanente):
    resultado = context.elements._implicity_wait(time=os.getenv("TIME_IMPLICITY"), element_by=PageModel.txt_result_form).text
