...
def combine_allure(folder):
    ...
    for file in files_should_be:
        ...
        if not os.path.exists(path):
            raise Exception(f"ERROR: File {folder + os.sep + file} doesnt exists, but it should!")
    ...
    for path, dirs, files in os.walk(folder):
        if files:
            regex = f"^{folder.rstrip('/')}" + os.sep
            regex = regex.replace('\\', "\\\\")

            folder_url = re.sub(regex, "", path)
            ...
            if folder_url and folder_url != folder:
                for file in files:
                    ...
                    if ext not in allowed_extensions:
                        print(
                            f"WARNING: Unsupported extension: {ext} (file: {path}/{file}) skipping (supported are: {allowed_extensions}")
                        continue
    ...
    with open(folder + os.sep + "server.js", "w", encoding="utf8") as f:
        f.write(r"""
        ...
        for d in data:
            ...
            if b64:
                ...
                aux_content = str(d['content'])
                content = aux_content.replace("\\", "\\\\").replace('"', '\\"').replace("\n", "\\n")
                f.write(f""" "{url}": "{content}", \n""")
        f.write("};\n")
    ...
    size = os.path.getsize(folder + os.sep + 'server.js')
    print(f"server.js is build, it's size is: {size} bytes")

    print("> Copying file sinon-9.2.4.js into folder...")
    copyfile(cur_dir + os.sep + "sinon-9.2.4.js", folder + os.sep + "sinon-9.2.4.js")

    print("sinon-9.2.4.js is copied")

    print("> Reading index.html file")
    with open(folder + os.sep + "index.html", "r", encoding="utf8") as f:
        index_html = f.read()
    ...
    if "sinon-9.2.4.js" not in index_html:
        ...
        with open(folder + os.sep + "index.html", "w", encoding="utf8") as f:
    ...
    with open(folder + os.sep + "hakalab_report.html", "w", encoding="utf8") as f:
        f.write(str(soup))

    print(f"> Saving result as {folder} {os.sep}hakalab_report.html")

    size = os.path.getsize(folder + os.sep + 'hakalab_report.html')
...
if __name__ == '__main__':
    ...
    combine_allure(args.folder.rstrip(os.sep))
